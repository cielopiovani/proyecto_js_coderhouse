...
const productos = [
    {
      ...
      descripcion: "Breteles anchos y cómodos, excelente calidad. Ideal para usar en el verano como prenda exterior, y para prácticar deporte.",
      stock: "SIN STOCK",
    },
    {
      id: "vedetinaMNatural",
  ...
  function cargarProductos() {
    productos.forEach((producto) => {
      ...
      // Pasar el contenido (del HTML) creándolo desde JS -> Incorporar el contenido
  ...
  // Función con evento en los botones para agregar al carrito
  ...
  const pedidoLS = localStorage.getItem("pedido-carrito");
  
  if (pedidoLS) {
    pedido = JSON.parse(pedidoLS);
  ...
  function agregarAlCarrito(e) {
    ...
    const productoAgregado = productos.find(
      (producto) => producto.id === idBotonProducto
    );
    ...
    if (pedido.some((producto) => producto.id === idBotonProducto)) {
      ...
      const productoConCantidad = { ...productoAgregado, cantidad: 1 };
      pedido.push(productoConCantidad);
    }
    actualizarNumeroPedido();
  ...
  function actualizarNumeroPedido() {
    let numeroPedidoActualizado = pedido.reduce(
